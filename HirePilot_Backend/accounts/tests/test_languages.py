...
class TestLanguageFlow:
    ...
    def user(self):
        ...
        return User.objects.create_user(
            email='polyglot@test.com', password='password123', is_verified=True
        )
    ...
    def test_create_language_full(self, auth_client, user):
        ...
        data = {'language': 'English', 'level': 'C2', 'certificate': 'Cambridge Proficiency (CPE)'}
    ...
    def test_get_own_languages_only(self, auth_client, user):
        ...
        Language.objects.create(user=user, language='French', level='B2')

        # 2. Idioma de OTRO usuario
        other_user = User.objects.create_user(email='other@test.com', password='pwd')
        Language.objects.create(user=other_user, language='German', level='A1')
    ...
    def test_update_language(self, auth_client, user):
        ...
        lang = Language.objects.create(user=user, language='Italian', level='A1')
    ...
    def test_delete_language(self, auth_client, user):
        ...
        lang = Language.objects.create(user=user, language='Japanese', level='N5')
