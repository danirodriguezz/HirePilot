...
class TestEducationFlow:
    ...
    def user(self):
        ...
        return User.objects.create_user(
            email='student@test.com', password='password123', is_verified=True
        )
    ...
    def test_get_own_education_only(self, auth_client, user):
        ...
        Education.objects.create(
            user=user, institution='My Uni', degree='B.Sc.', start_date='2020-01-01'
        )

        # 2. Educación de OTRO usuario
        other_user = User.objects.create_user(email='other@test.com', password='pwd')
        Education.objects.create(
            user=other_user, institution='Other Uni', degree='PhD', start_date='2020-01-01'
        )
    ...
    def test_update_education(self, auth_client, user):
        ...
        edu = Education.objects.create(
            user=user, institution='College', degree='Associate', current=True
        )
    ...
    def test_delete_education(self, auth_client, user):
        ...
        edu = Education.objects.create(user=user, institution='To Delete', degree='B.A.')
    ...
    def test_ordering(self, auth_client, user):
        ...
        # 2010 (Viejo)
        Education.objects.create(
            user=user, institution='High School', degree='HS', start_date='2010-01-01'
        )
        # 2020 (Nuevo)
        Education.objects.create(
            user=user, institution='University', degree='M.Sc.', start_date='2020-01-01'
        )
